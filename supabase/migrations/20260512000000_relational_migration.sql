...
--
-- À appliquer via : supabase db push  (nécessite DB_PASSWORD)
-- Ou via le Dashboard Supabase > SQL Editor
-- ============================================================================

-- ── 1. Table `transactions` (données comptables) ──────────────────────────────
-- Remplace la sous-clé `donneesMensuelles[mois].transactions` dans app_state.
...
CREATE TABLE IF NOT EXISTS public.transactions (
  ...
  piece_jointe    text,               -- data URL base64 (migration future vers Storage)
...
-- Remplace la sous-clé `donneesMensuelles[mois].retenues` dans app_state.
...
-- Remplace la clé `s:<id>:sanctions` dans app_state.
...
-- Réutilise la fonction monostable déjà présente dans le schéma.
...
-- ── 5. Realtime pour les nouvelles tables ─────────────────────────────────────
-- (optionnel — à activer si le Realtime est nécessaire)
-- ALTER PUBLICATION supabase_realtime ADD TABLE public.transactions;
-- ALTER PUBLICATION supabase_realtime ADD TABLE public.retenues;
-- ALTER PUBLICATION supabase_realtime ADD TABLE public.sanctions;

-- ── 6. Script de backfill (à exécuter après l'activation) ────────────────────
-- Le backfill des données depuis app_state vers les tables relationnelles
-- sera réalisé par un script Node.js séparé (src/scripts/backfill.ts)
-- pour pouvoir être exécuté de façon incrémentale et vérifiée.
--
-- Ordre recommandé :
--   1. employes         (clé s:<id>:employes → table employes)
--   2. articles + fournisseurs + categories_stock
--   3. immobilisations
--   4. params_annuels
--   5. donneesMensuelles → absences, heures_sup, primes, factures, devis,
--                          mouvements_stock (déjà dans tables ✅)
--   6. transactions (nouvellement créée)
--   7. sanctions, retenues (nouvellement créées)
-- ============================================================================
